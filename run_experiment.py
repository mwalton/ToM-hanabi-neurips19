...
import gin.tf
import sys
sys.path.insert(0, '/hanabi/')
import rl_env
import numpy as np
import rainbow_agent
import tensorflow as tf
from belief_model import HandBeliefModel
...
@gin.configurable
def create_obs_stacker(environment, belief_model, history_size=4):
  ...
  return ObservationStacker(history_size,
                            environment.vectorized_observation_shape()[0] + belief_model.belief_size(),
                            environment.players)

@gin.configurable
def create_belief_model(belief_level, belief_mod, num_players, num_b0_samples, comms_reward=False, beta=0.0):
  return HandBeliefModel(belief_level, belief_mod, num_players, num_b0_samples=num_b0_samples, comms_reward=comms_reward, beta=beta)


@gin.configurable
def create_agent(environment, obs_stacker, agent_type='DQN', tf_device='/cpu:*'):
...
def initialize_checkpointing(agent, experiment_logger, checkpoint_dir,
  ...
  if latest_checkpoint_version >= 0:
    ...
    if agent.unbundle(checkpoint_dir, latest_checkpoint_version, dqn_dictionary):
...
def parse_observations(observations, num_actions, obs_stacker, belief_model):
  ...
  observation_vector = belief_model.modify_observation(observations, current_player_observation['vectorized'], current_player)
...
def run_one_episode(agent, environment, obs_stacker, belief_model):
  ...
  current_player, legal_moves, observation_vector = (
      parse_observations(observations, environment.num_moves(), obs_stacker, belief_model))
  action = agent.begin_episode(current_player, legal_moves, observation_vector)

  is_done = False
  total_reward = 0
  total_comms_reward = 0
  step_number = 0
  ...
  observation_vector_old = observation_vector
  ...
  while not is_done:
    ...
    if belief_model.comms_reward:
      comms_rew = belief_model.get_comms_reward(observation_vector_old, observation_vector)
      total_comms_reward += comms_rew
    else:
      comms_rew = 0
    ...
    if current_player in has_played:
      action = agent.step(reward_since_last_action[current_player] + comms_rew,
                          current_player, legal_moves, observation_vector)
    else:
  ...
  if agent.eval_mode:
    ...
    return step_number, total_reward, total_comms_reward, move_counter


def run_one_phase(agent, environment, obs_stacker, belief_model, min_steps, statistics,
                  run_mode_str):
  ...
  while step_count < min_steps:
    episode_length, episode_return, comms_episode_return, move_counter = run_one_episode(agent, environment,
                                                     obs_stacker, belief_model)
    statistics.append({
        '{}_episode_lengths'.format(run_mode_str): episode_length,
        '{}_episode_returns'.format(run_mode_str): episode_return,
        '{}_epsisode_comms_returns'.format(run_mode_str): comms_episode_return,
        '{}_move_counts'.format(run_mode_str): move_counter
    })
...
@gin.configurable
def run_one_iteration(agent, environment, obs_stacker, belief_model,
...
def run_experiment(agent,
  ...
  for iteration in range(start_iteration, num_iterations):
    start_time = time.time()
    statistics = run_one_iteration(agent, environment, obs_stacker, belief_model, iteration,
                                   training_steps)
